#!/usr/bin/env python3
"""
Benchmark Log Analyzer with Line Plots

This script reads benchmark log files from night_logs_2 and generates line plots using Plotly
...
class BenchmarkLogAnalyzer:
    def __init__(self, log_file: str = "./night_logs_2"):
    ...
    def plot_throughput_by_quantization(self, df: pl.DataFrame, output_dir: str) -> None:
        """Create line plot of throughput vs quantization."""
        if df.is_empty():
            return
    ...
    def plot_throughput_by_model_size(self, df: pl.DataFrame, output_dir: str) -> None:
        """Create line plots of throughput vs model size - one plot per model family with quantization as colors."""
        if df.is_empty():
            return


        def extract_model_family(model_name: str) -> str:
            """Extract base model family from model filename."""
    ...
    def plot_throughput_by_device(self, df: pl.DataFrame, output_dir: str) -> None:
        """Create line plots of throughput vs device type - one plot per model + quantization combination with model size as colors."""
        if df.is_empty():
            return
        ...
        def get_device_sort_key(device_type: str) -> tuple:
            """Return a sort key for logical device type ordering."""
    ...
    def plot_throughput_by_batch_size(self, df: pl.DataFrame, output_dir: str) -> None:
        """Create line plots of throughput vs batch size - one plot per model with device configuration as colors."""
        if df.is_empty():
            return
    ...
    def plot_throughput_vs_batch_size_by_prompt_length(self, df: pl.DataFrame, output_dir: str) -> None:
        """Create line plots of throughput vs batch size for each model with prompt length as color dimension."""
        if df.is_empty():
            return
    ...
    def run_analysis(self, output_dir: str) -> None:
        """Run the complete analysis pipeline with line plots."""
...
if __name__ == "__main__":
    ...
    parser.add_argument('--log_file', '-l', default='./night_logs_2',
                       help='Path to the log file to analyze (default: ./night_logs_2)')
